...
interface Background { type: 'image' | 'gradient'; value: string; label: string; }
...
const BACKGROUNDS: Background[] = [
  ...
  { type: 'gradient', value: 'linear-gradient(135deg,#300a24 0%,#4a1042 15%,#5d0e41 30%,#c0392b 55%,#e95420 75%,#f4a95a 100%)', label: 'Ubuntu Gradient' },
  { type: 'gradient', value: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)', label: 'Midnight Blue' },
];

@Component({
  selector: 'app-desktop',
  imports: [TopBarComponent, BottomBarComponent, WindowManagerComponent, ContextMenuComponent],
...
export class DesktopComponent {
  ...
  get currentBg(): Background { return this.backgrounds[this.bgIndex()]; }
  get bgStyle(): string {
  ...
  @HostListener('click')
  onDesktopClick(): void { this.ctxVisible.set(false); }
